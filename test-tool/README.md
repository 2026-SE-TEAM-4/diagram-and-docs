...
`test-plan.html`(테스트 계획서)에서 정의한 테스트들을 **실제로 어떤 도구로, 어떤 구조로 구현할지** 설계한 문서 모음입니다.
...
```text
2026-se-team-4/
├── testing/                  # testkit CLI (00~03) — uv run testkit ...
...
5. **결과 저장 규약** — `testing/results/<날짜시각>-<명령>/` 에 summary.json + report.txt + 엔진 원본. testkit이 자동으로 처리.
