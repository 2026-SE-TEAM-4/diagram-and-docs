...
| models (ORM) | `app/models/` | 17개 엔티티의 테이블 매핑과 도메인 열거형(enums). 비즈니스 로직 없음 | database(Base)만 |
...
flowchart TD
    ...
    MODELS["models (ORM 엔티티 17개) + schemas (Pydantic DTO)"]
...
`app/models/__init__.py`가 17개 엔티티를 한곳에서 import해 `Base.metadata`에 등록한다.
...
`app/jobs/scheduling.py`의 `register_jobs`에서 interval 잡(`id="reservation_transitions"`)으로
등록된다. 주기는 로컬 데모용으로 가속한 5초다(설계 주기는 1분). 한 번의 실행이 두 종류의
전이를 순서대로 처리하고 마지막에 한 번 커밋한다.

```text
매 주기 (process_reservation_transitions):
  새 DB 세션 열기
  try:
...
마찬가지로 `app/jobs/scheduling.py`의 `register_jobs`에서 등록된다(`id="approval_timeout"`,
데모 5초 / 설계 1분). 타임아웃 상수는 `_TIMEOUT = timedelta(hours=72)`(`approval_jobs.py:18`)이다.

```text
매 주기 (auto_reject_timed_out_requests):
  새 DB 세션 열기
  try:
    now    ← 현재 UTC
    cutoff ← now - 72시간
...
### 5.4 AIOps 잡 (F24·F27·F28·F30~F34)
...
주기는 **로컬 데모용으로 가속한 초 단위**다(괄호 안은 원래 설계 주기). 단일 출처는
`scheduling.py`이며, 잡 id·주기는 이 파일과 정확히 일치한다.

| 잡 id (파일) | 주기(데모/설계) | 기능 (기능 ID / 유스케이스) | 산출물 |
|---|---|---|---|
| `metric_collection` (`metric_collection_job`) | 5초 / 1분 (P0 / UC14) | 서버 풀의 `/metrics` 엔드포인트를 PULL해 ServerMetric 저장. 무응답·계약 위반 서버는 `status=MISSING`으로 기록 | ServerMetric 행 |
| `anomaly_detection` (`anomaly_detection_job`) | 5초 / 1분 (F27 / UC18) | 활성 서버의 최근 7일 OK 메트릭으로 메트릭별(CPU/MEM/NET/GPU) 기준선(μ±kσ, k=2.5) 이탈 탐지. 표본 30개 미만·σ 0이면 비이상, 같은 서버·메트릭은 1시간 디바운스 | AnomalyRecord 행 |
| `health_score` (`health_score_job`) | 10초 / 10분 (F28 / UC19) | 서버별 건강 점수 계산 후 `Server.health_score` 갱신 및 ServerHealthHistory 적재. `Server.version`을 건드리지 않는 직접 UPDATE로 낙관적 락 충돌 없음 | `Server.health_score` 갱신, ServerHealthHistory 행 |
| `incident_correlation` (`incident_correlation_job`) | 5초 / 5분 (F33 / UC24) | 미할당 이상(최근 10분)을 서버 그룹으로 묶어 Incident 생성·부착. 새 인시던트일 때만 ADM에게 INCIDENT 알림 1건. 15분 유휴 시 RESOLVED 자동 종료 | Incident 행, Notification |
| `forecast` (`forecast_job`) | 30초 / 60분 (F31 / UC22) | Holt-Winters 지수평활법으로 자원 사용량·예약 수요 추세 예측 및 Forecast 저장. 포화 72시간 이내 예상 시 ADM에게 CAPACITY 알림 | Forecast 행, Notification |
| `incident_summary` (`incident_summary_job`) | 10초 / 5분 (F34 / UC25) | 요약 없는 OPEN 인시던트를 Gemini API로 분석해 IncidentSummary 생성(인시던트당 1회). API 키가 없으면 graceful skip | IncidentSummary 행 |
| `failure_prediction` (`failure_prediction_job`) | 15초 / 10분 (F32 / UC23) | 7일 건강점수 기울기(최소제곱)·24h 이상 빈도·현재 건강을 가중합한 위험도로 `Server.risk_score`·`Server.eta_to_risk` 갱신. 위험도 50 이상이면 ADM에게 PREDICTIVE_FAILURE 알림. `Server.version`을 건드리지 않는 직접 UPDATE | `Server.risk_score`·`eta_to_risk` 갱신, Notification |
| `idle_reclaim` (`idle_reclaim_job`) | 5초 / 1분 (F24 / UC15) | 유휴(저사용률) 점유 서버를 경고(IDLE_WARNING) 후 유예가 지나면 자동 회수(예약 RECLAIMED, 서버 AVAILABLE+version, Quota.used 감소) | Reservation·Server·Quota 갱신, Notification |
| `maintenance_transition` (`maintenance_transition_job`) | 5초 / 1분 (F30 / UC13) | 점검 일정 도래에 맞춰 서버를 MAINTENANCE/AVAILABLE 로 전환(version+1) | Server 상태 전환 |

**설계 참고 사항**

- 낙관적 락 안전: `health_score`·`risk_score`·`eta_to_risk`는 `Server.version`을 증가시키지 않는 직접 UPDATE로 갱신한다. 예약·즉시 할당 경로의 낙관적 락과 충돌하지 않으면서도 AIOps 값이 지속적으로 최신 상태를 유지한다. 반면 상태 전이 잡(`idle_reclaim`·`maintenance_transition`)은 예약 흐름과 일관되게 `version+1`을 함께 올린다.
- 안정 풀 오탐 억제: 이상탐지는 단순 μ±2σ가 아니라 k=2.5σ를 쓰고, 판정용 σ에 절대 하한(`_MIN_SIGMA=8.0`)을 둔다. 매우 안정적인 구간은 σ가 0에 가까워 밴드가 면도날처럼 좁아지는데, 하한이 없으면 평소 잔떨림(예: GPU 합성값 ±5)조차 이상으로 오탐한다. 표본 30개 미만이거나 σ가 0이면 항상 비이상으로 판정해, 유휴·안정 서버 풀이 초기에 헛인시던트·저건강점수를 내지 않게 한다(기록되는 `stddev`는 실제 σ 그대로).
