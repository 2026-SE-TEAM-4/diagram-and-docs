...
| jobs (스케줄러) | `app/jobs/` | APScheduler 주기 잡(예약 자동 전이, 승인 자동 거절). api를 거치지 않고 services와 같은 높이에서 models에 접근 | models, database |

진입점은 두 개다. `app/main.py`(FastAPI 앱 + lifespan에서 APScheduler 잡 등록)와
`app/scheduler.py`(별도 잡 컨테이너용 엔트리포인트)로, 한 코드베이스에서 두 프로세스를 운영한다.

```mermaid
flowchart TD
    subgraph entry["진입점"]
        MAIN["main.py (FastAPI 앱)"]
        SCHED["scheduler.py (잡 컨테이너)"]
    end
    ...
    MODELS["models (ORM 엔티티 13개) + schemas (Pydantic DTO)"]
...
`app/models/__init__.py`가 13개 엔티티를 한곳에서 import해 `Base.metadata`에 등록한다.
...
| 4 | Server | `server` | `status`(AVAILABLE/RESERVED/IN_USE/MAINTENANCE), **`version`(낙관적 잠금)**, `health_score`, `deleted_at`(soft delete) |
...
| 9 | AnomalyRecord | `anomaly_record` | `server_id → server.id`, `current_value`/`mean`/`stddev` 기반 이상 징후 이력 |
...
Quota 초과 시 ApprovalRequest가 우회 경로를 제공하며, 나머지 엔티티(메트릭·이상징후·점검·대기열·로그)는
...
`app/main.py:22`에서 1분 주기 interval 잡으로 등록된다.
...
마찬가지로 `app/main.py:24`에서 1분 주기로 등록된다. 타임아웃 상수는
