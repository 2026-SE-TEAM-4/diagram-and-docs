...
| 단계 | 현업 패턴 | 우리 현황 |
|------|-----------|-----------|
| 탐지 | 동적 이상탐지 | 있음 — 정적 통계(μ±kσ, k=2.5, σ 하한 8.0) 수준 (F27/UC18)(구현됨) |
...
### B-2. 필요 기술
- **추세 분석:** healthScore 시계열의 기울기(구현은 최소제곱 직선 적합. 10분 간격 표본의
  잔떨림을 하루로 외삽해 부풀리지 않도록 ±20/일로 제한).
- **이상 빈도 집계:** 최근 24h `AnomalyRecord` 발생 빈도.
...
| F32 | 장애·건강 열화 예측 | UC23 | 스케줄러 10m | 최소제곱 추세 기울기, (로지스틱) | Server 확장 | GET /servers/{id}/health-trend |
